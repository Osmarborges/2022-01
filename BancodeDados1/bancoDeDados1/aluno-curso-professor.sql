...
--   select * from aluno;
--   select nomeAlun from aluno where idade < 20 and nivel = 'JR';   
...
select A.nomeAlun from aluno as A, matriculado as M where A.nroAlun = M.nroAlun and M.nomeCurso  = 'Database Systems';
...
SELECT NIVEL FROM ALUNO AS ALUNO A, MATRICULADO M,CURSO C
WHERE A.nroAlun, M.NROALUN AND
M.NOMECURSO = C.NOME AND
C.SALA ='20 AVW';

-- SELECT distinct nomeAlun, nivel, curso.sala FROM aluno , curso  where  sala ='20 AVW';
select * from curso;
select * from professor;
select * from aluno;
select * from matriculado;
-- Liste o nome do alunos e o nome da formação cuja formação contenha a string 'cie'
select nomeAlun, formacao from aluno
where formacao LIKE '%cie%';
-- LIKE '%science'
-- 'Computer%'
-- LIKE 'L%w' pega qualquer caractere
-- 'L_W' obriga ter letra

-- Conte o número de alunos.
select count(*) from aluno;
select count(*) AS QTDE_ALUNOS from aluno; -- "QTD ALUNO" AS ASPAS PARA TER O ESPAÇO
SELECT SUM(idade) FROM ALUNO;
SELECT SUM(idade)/COUNT(*) FROM ALUNO;
SELECT AVG(IDADE) FROM ALUNO; -- MEDIAS DOS ALUNOS
...
select * from aluno;
select * from matriculado;
SELECT count(*)matriculado FROM MATRICULADO
...
select nomeProf from professor, curso
