...
-- 1) Selecione todos alunos que são menores do que 20 anos e são do nível 'JR'

SELECT * 
FROM ALUNO 
WHERE nivel = 'JR' AND idade < 20;
-- 2) Selecione os professores que são do departamento  68

SELECT * 
FROM PROFESSOR 
WHERE idDepto = 68;
-- 3) Selecione os alunos que se matricularam em curso de 'Database Systems'

SELECT * 
FROM ALUNO, MATRICULADO 
WHERE MATRICULADO.nroAlun = ALUNO.nroAlun AND 
     nomeCurso = 'Database Systems';
-- 4) Selecione os nomes dos professores que ministraram algum curso na sala '20 AVW'

SELECT nomeProf 
FROM PROFESSOR, CURSO 
WHERE CURSO.idProf = PROFESSOR.idProf AND 
     sala = '20 AVW';
-- 5) Selecione o nível dos alunos (sem repetição) que cursaram algum curso na sala '20 AVW'
...
WHERE A.nroAlun = M.nroAlun AND 
     ...
     C.sala = '20 AVW';
-- 6) Liste o nome do alunos e o nome da formação cuja formação contenha a string 'cie'

SELECT nomeAlun, formacao 
FROM ALUNO 
WHERE formacao LIKE '%cie%';
-- 7) Conte o número de alunos.

SELECT COUNT(*) AS QtdeAluno FROM ALUNO;
-- 8) Some as idades de todos alunos.

SELECT SUM(idade) FROM ALUNO;
-- 9) Mostre a idade média de todos alunos.

SELECT SUM(idade)/COUNT(*), AVG(idade) FROM ALUNO;
-- 10) Mostre a idade máxima e a idade mínima de todos alunos.

SELECT MAX(idade), MIN(idade) FROM ALUNO;
-- 11) Quantas matrículas há no curso com nome 'Database Systems'

SELECT COUNT(*) 
FROM MATRICULADO 
WHERE nomeCurso = 'Database Systems';
-- 12) Selecione o nome de todos os professores dos departamentos 68 e 12.
...
-- 13) Selecione o nome dos professores que deram aulas no curso com 'Database Systems' ou são do departamento 68.
...
-- 14) Selecione o número dos alunos com formação 'Computer Science' ou que fizeram o curso de 'Database Systems'.
